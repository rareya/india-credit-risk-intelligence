...
**Important distinction** (this is easy to misread — the two of us did, briefly): `final_model_selection.py` does **not** train or persist the final model. It only *decides* which candidate wins. The actual model artifact is built by a separate script two stages later. Full chain:

| Stage | Script | Does | Does NOT |
|---|---|---|---|
| Feature registry | `feature_governance.py` | Classifies every gold-layer feature into excluded / review / conservative tiers | — |
| Semantic direction check | `feature_semantics_audit.py` | PASS — 0 direction mismatches, 0 strong single-feature signals among conservative features | — |
| Prediction-time provenance | `prediction_time_provenance.py` | PASS — 30/30 conservative features | Verify the real production data feed's timing |
| Model comparison | `run_ml_model.py` | Trains baseline / conservative / expanded, produces `model_comparison.parquet` | Pick a winner |
| **Governance decision** | **`final_model_selection.py`** | **Rejects the expanded model, decides conservative wins, writes `final_model_summary.json`** | **Train or persist any model artifact** |
| Frozen config | `final_model_config.py` | Locks the exact approved feature set + hyperparameters (`EXCLUDED_FEATURES` includes `cibil_score`, `cibil_band` — matches the decision above) | — |
| **Final artifact build** | **`train_final_model.py`** | **Fits the frozen config on 100% of gold data, saves `conservative_xgb_v1.joblib` + metadata** | Select or tune the model — config is already frozen |
| Scoring | `score_final_model.py` | Scores the portfolio with the frozen artifact | Select / train / tune / evaluate generalization |
...
**On `final_model_summary.json` specifically:** this file is generated by exactly one script — `final_model_selection.py` — and it records the *governance decision*, not the model itself. If this file's `decision` field ever looks wrong or stale, the fix is always to re-run that one script — never to hand-edit the JSON, since the next run will overwrite manual edits anyway. It will never contain model metrics or the artifact path; that's `train_final_model.py`'s output (`src/modeling/final_model/model_metadata.json`), a different file entirely.
